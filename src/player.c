...
void checkCoinCollision(Game *game, Player *player, Level *level) {
...
void updatePlayer(Player *p, Level *level, Game *game) {
    ...
    p->x += p->dx; 
    checkCoinCollision(game, p, level);
    ...
    if (checkWin(p, level)) {
        ...
        if (game->currentLevel > 3) {
            ...
            game->finalTime = (SDL_GetTicks() - game->gameStartTime) / 1000.0f;
            Mix_HaltMusic();
            printf("[FINAL] The game is fully completed!\n");
            printf("Total time: %.2f seconds\n", game->finalTime);
            
            if (game->winSound) 
                Mix_PlayChannel(-1, game->winSound, 0);
            ...
            initPlayer(p, level->spawnX, level->spawnY); 
                 ...
                 if (game->currentLevel == 2 && game->level2Music) 
                     Mix_PlayMusic(game->level2Music, -1);
            
                 else if (game->currentLevel == 3 && game->level3Music) 
                     Mix_PlayMusic(game->level3Music, -1);
