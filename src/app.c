...
bool initApp(App *app) {
    ...
    app->plusButton = IMG_LoadTexture(app->renderer, "resource/images/plus.png");
    if (!app->plusButton)
        printf("Failed to load plus button: %s\n", IMG_GetError());

    app->minusButton = IMG_LoadTexture(app->renderer, "resource/images/minus.png");
    if (!app->minusButton)
        printf("Failed to load minus button: %s\n", IMG_GetError());
...
void cleanupApp(App *app) {
    ...
    if (app->plusButton)
        SDL_DestroyTexture(app->plusButton);

    if (app->minusButton)
        SDL_DestroyTexture(app->minusButton);
